...
describe('i18n Property Tests', () => {
  ...
  /**
   * Property: each locale carries its correct text direction.
   * Arabic (ar) is RTL; every other supported locale is LTR.
   */
  it('each locale has the correct text direction (ar=rtl, rest=ltr)', () => {
    fc.assert(
      fc.property(
        fc.constantFrom(...SUPPORTED_LOCALES),
        (locale) => {
          const expected = locale === 'ar' ? 'rtl' : 'ltr';
          return LOCALE_CONFIG[locale].direction === expected;
        }
      ),
      { numRuns: 100 }
    );
